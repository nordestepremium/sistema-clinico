...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await queryComoClinica(
      req.clinicaId,
      isRecepcao
        ? 'SELECT * FROM pacientes WHERE clinica_id=$1 ORDER BY nome_completo'
        : 'SELECT * FROM pacientes WHERE usuario_id=$1 AND clinica_id=$2 ORDER BY nome_completo',
      isRecepcao ? [req.clinicaId] : [req.usuarioId, req.clinicaId]
    );
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (req.role === 'recepcao') {
      ...
      const dono = await queryComoClinica(
        req.clinicaId,
        "SELECT id FROM usuarios WHERE id=$1 AND role != 'recepcao' AND clinica_id=$2",
        [p.usuario_id, req.clinicaId]
      );
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    await queryComoClinica(
      req.clinicaId,
      'DELETE FROM pacientes WHERE id=$1 AND usuario_id=$2 AND clinica_id=$3',
